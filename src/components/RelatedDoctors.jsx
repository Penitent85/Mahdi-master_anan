...
const RelatedDoctors = ({ speciality, docId }) => {
  ...
  useEffect(() => {
    // Fetch doctors' data on component mount or when `speciality` or `docId` changes
    const fetchDoctors = async () => {
      const filteredDoctors = doctors?.filter(
        (doc) => doc.speciality === speciality && doc._id !== docId
      );
      setRelDoc(filteredDoctors); // Set filtered doctors
    };

    fetchDoctors();
  }, [docId,speciality]); // Dependency array
